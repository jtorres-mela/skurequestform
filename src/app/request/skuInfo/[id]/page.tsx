import ManageSKUActions from "@/lib/components/ManageSKUActions";
...
export default async function SkuInfoPage(context: { params: { id: string } }) {
  ...
  const productData = await prisma.submissionProduct.findUnique({
  ...
  // Convert Decimal fields to strings to make the object serializable for Client Components
  const product = {
    ...productData,
    markets: productData.markets.map((market) => ({
      ...market,
      savings: market.savings?.toString() ?? null,
    })),
  };
    ...
    <>
      <div className="flex justify-end mb-4 max-w-7xl mx-auto">
        <ManageSKUActions
          ...
          requestId={product.submission.id}
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm max-w-7xl mx-auto">
        <div className="flex justify-between">
          ...
          <div>
            <Link
              href={`/request/${product.submission.requestId}`}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <tbody className="divide-y divide-gray-200">
                ...
                {renderKeyValueRow(
                  "PDP Requested",
                  product.isPdpRequested ? "Yes" : "No"
                )}
                {product.isPdpRequested &&
                  renderKeyValueRow("PDP Work Request", product.pdpWorkRequest)}
              </tbody>
            </table>
          </div>
        )}

        {product.includeTranslations &&
                  ...
                  {product.cultures.map((culture) => (
                    <tr key={culture.id}>
                      ...
                      <td className="px-4 py-3">
                        {culture.translatedName || "N/A"}
                      </td>
                      <td className="px-4 py-3">
                        {culture.translatedShort || "N/A"}
                      </td>
                      <td className="px-4 py-3">
                        {culture.translatedLong || "N/A"}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {product.accessories.map((accessory) => (
                    <tr key={accessory.id}>
                      <td className="px-4 py-3 whitespace-nowrap">
                        {accessory.accessorySku || "N/A"}
                      </td>
                      <td className="px-4 py-3">
                        {accessory.accessoryLabel || "N/A"}
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody className="divide-y divide-gray-200">
                  {product.markets.map((market) => (
                    <tr key={market.id}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {market.onSaleDate
                          ? formatDateAsUTC(market.onSaleDate)
                          : "—"}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        {market.noEndDate
                          ? "No End Date"
                          : market.offSaleDate
                          ? formatDateAsUTC(market.offSaleDate)
                          : "—"}
                      </td>
                    </tr>
                  ))}
